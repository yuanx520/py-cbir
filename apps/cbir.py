    ...
    def match3(self, img_dst, obj, f_func, h_func, d_func, thres=0.3):
        ...
        for f in F:
            ...
            for path in obj[code]:
                ...
                if min_d < thres:
                    match_dict[path] = match_dict.get(path, 0) + 1
        sort_list = sorted(match_dict.items(),
                           key=lambda d: d[1], reverse=True)
        return [(k, repr(v)) for k, v in sort_list[:5]]

    def search(self, dst_thum, debug=False):
        hog_list = self.match(dst_thum, self.hog_index,
                              hog3, LSH_hog, eculidean_dist)
        #hog_list2 = self.match2(dst_thum, self.hog_index, hog3, LSH_hog, eculidean_dist)
        sift_list = self.match(dst_thum, self.sift_index,
                               sift2, LSH_sift, eculidean_dist)
...
class GlobalMatcher(object):
    ...
    def search(self, dst_thum, debug=False):
        ...
        ghisto_list = self.match(
            dst_thum, self.grayhisto, gray_histo, abs_dist)
    ...
    def post(self):
        ...
        if self.request.files:
            ...
            if extension[1:] not in EXTS:
                ...
                self.render('template_cbir.html', err_msg=err_msg,
                            imgpath=img_path, lists=[])
